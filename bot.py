...
async def _live_match_simulation(context, team1_name, team1_squad, team1_form,
    ...
    """Tek bir maçı canlı simüle et — dakika dakika mesajlar."""
    # Taktik bilgileri context.bot_data'dan alınamaz, parametre olarak gelmeli
    # Geriye dönük uyumluluk için: eğer simulate_match_with_tactics çağrılabilirse onu kullan
    try:
        g1, g2, events, mvp, scorers = lig.simulate_match_with_tactics(
            team1_name, team1_squad, team1_form,
            getattr(_live_match_simulation, "_t1_form", "4-3-3"),
            getattr(_live_match_simulation, "_t1_tactic", "dengeli"),
            team2_name, team2_squad, team2_form,
            getattr(_live_match_simulation, "_t2_form", "4-3-3"),
            getattr(_live_match_simulation, "_t2_tactic", "dengeli"),
        )
    except Exception:
    ...
    intro = (
        f"⚽ *MAÇ BAŞLIYOR!*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🏠 *{team1_name}* {'📈' if team1_form > 0 else '📉' if team1_form < 0 else '➡️'}\n"
        f"🆚\n"
        f"🛫 *{team2_name}* {'📈' if team2_form > 0 else '📉' if team2_form < 0 else '➡️'}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🕐 1. Yarı başladı..."
    )
    await _send_to_broadcast_chats(context, intro)

    # Olayları sırayla gönder (3 sn aralıklı)
    current_g1, current_g2 = 0, 0
    for evt in events[:8]:  # Max 8 olay göster (spam olmasın)
        ...
        if evt_type == "goal":
            if team == team1_name:
                ...
                current_g2 += 1
            score = f"{current_g1}-{current_g2}"
            msg = f"⚽ *{minute}\'* GOOOL! *{player}* ({team})\n"
            if detail:
                msg += f"   👟 Asist: _{detail}_\n"
            msg += f"   📊 *{team1_name}* `{score}` *{team2_name}*"
            await _send_to_broadcast_chats(context, msg)
        elif evt_type == "yellow":
            await _send_to_broadcast_chats(context,
                f"🟨 *{minute}\'* Sarı kart: _{player}_ ({team})")
    ...
    if scorers["team1"]:
        final += f"\n🥅 *{team1_name}:*\n"
        for s in scorers["team1"]:
            final += f"  • {s['minute']}\' {s['player']}"
            if s.get('assist'):
                final += f" (asist: {s['assist']})"
            final += "\n"

    if scorers["team2"]:
        final += f"\n🥅 *{team2_name}:*\n"
        for s in scorers["team2"]:
            final += f"  • {s['minute']}\' {s['player']}"
            if s.get('assist'):
                final += f" (asist: {s['assist']})"
            final += "\n"
    ...
    await _send_to_broadcast_chats(context, final)
...
async def daily_match_job(context):
    ...
    for t1, t2 in pairs:
        ...
        # Sosyal medya tepkileri (rastgele 1-2 oyuncu hakkında)
        try:
            import random as _rsoc
            # Kazanan takım: pozitif tepki
            if g1 > g2 and scorers.get("team1"):
                p = _rsoc.choice(scorers["team1"])
                db.post_social_reaction(t1["name"], p["player"], "positive")
            elif g2 > g1 and scorers.get("team2"):
                p = _rsoc.choice(scorers["team2"])
                db.post_social_reaction(t2["name"], p["player"], "positive")
            else:
                # Beraberlik: nötr
                all_starters = t1["squad"][:11] + t2["squad"][:11]
                if all_starters:
                    rand_p = _rsoc.choice(all_starters)
                    rand_team = t1["name"] if rand_p in t1["squad"][:11] else t2["name"]
                    db.post_social_reaction(rand_team, rand_p["name"], "neutral")
            # Kötü oynayan biri: negatif
            if g1 + g2 == 0:
                losing_squad = t1["squad"][:11]
                if losing_squad:
                    rand_p = _rsoc.choice(losing_squad)
                    db.post_social_reaction(t1["name"], rand_p["name"], "negative")
        except Exception:
            pass
...
async def bot_talk_job(context):
    ...
    try:
        ...
        msg = "🎰 *Cumhuriyetçiler Gazinosu çalışıyor!* /yardim ile komutları gör."
...
def main():
    ...
    # Maç öncesi duyurusu — her 5 dakikada kontrol (20:30'da çalışır)
    if app.job_queue:
        app.job_queue.run_repeating(prematch_announcement_job, interval=60, first=120, name="prematch")

    # Sezon bitiş kontrol — her saat
    if app.job_queue:
        app.job_queue.run_repeating(season_check_job, interval=3600, first=60, name="season_check")

    # Günlük lig maçları — her 24 saatte
    if app.job_queue:
        from datetime import time as dtime
        # TR 21:00 = UTC 18:00 (yaz saati uygulanmaz, Türkiye UTC+3 sabit)
        app.job_queue.run_daily(daily_match_job, time=dtime(hour=18, minute=0, tzinfo=timezone.utc), name="daily_matches")
